...
export default function IntegratedMarketDiscovery() {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Loading Overlay */}
      {loading && (
        <View style={[StyleSheet.absoluteFill, { backgroundColor: '#ffffff', justifyContent: 'center', alignItems: 'center', zIndex: 100 }]}>
      ...
      <View style={styles.overlayLayer}>
        ...
        <View style={styles.mapControls}>
             <TouchableOpacity style={styles.controlBtn} onPress={() => setRegion({ latitude: 12.9716, longitude: 77.5946, latitudeDelta: 0.2, longitudeDelta: 0.2 })}>
                <View style={[styles.controlBlur, { backgroundColor: '#ffffff' }]}>
                  <IconSymbol name="location.fill" size={22} color={c.primary} />
                </View>
             </TouchableOpacity>

             <View style={styles.legendWrapper}>
                <View style={[styles.legendBlur, { backgroundColor: '#ffffff' }]}>
           ...
           {marketTab === 'Mandi' ? (
               <FlatList 
                  ...
                  renderItem={({ item, index }) => (
                    <TouchableOpacity 
                      style={[
                        styles.marketMiniCard, 
                        selectedMarket?.id === item.id && { borderColor: c.primary, borderWidth: 1 }
                      ]} 
                      onPress={() => selectMarket(item)}
                    >
                        <View style={[styles.rankDot, { backgroundColor: index === 0 ? '#00C853' : '#f5f5f5' }]}>
                          <Text style={{ color: index === 0 ? '#fff' : '#666', fontSize: 10, fontWeight: 'bold' }}>#{index + 1}</Text>
                        </View>
      ...
      {selectedMarket && (
         <View style={styles.drawerContainer}>
            <View style={[styles.drawerCard, { backgroundColor: '#ffffff' }]}>
          ...
          <View style={styles.modalOverlay}>
              <View style={[styles.modalSheet, { backgroundColor: '#ffffff' }]}>
                  ...
                  <TouchableOpacity onPress={pickMachineImage} style={styles.imagePickerBtn}>
                      {newMachine.image ? <Image source={{ uri: newMachine.image || undefined }} style={styles.previewImg} /> : <IconSymbol name="camera.fill" size={40} color="#999" />}
                  </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  cropChip: { paddingHorizontal: 15, paddingVertical: 8, borderRadius: 20, backgroundColor: '#ffffff', marginRight: 10, borderWidth: 1, borderColor: '#ddd' },
  ...
  marketMiniCard: { width: 160, backgroundColor: '#ffffff', padding: 15, borderRadius: 20, marginRight: 15, elevation: 4, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 10, borderWidth: 1, borderColor: '#f0f0f0' },
  rankDot: { width: 22, height: 22, borderRadius: 11, justifyContent: 'center', alignItems: 'center', marginBottom: 8 },
  miniName: { fontSize: 13, fontWeight: 'bold', color: '#111' },
  miniPrice: { fontSize: 20, fontWeight: '900', color: '#00C853', marginTop: 5 },
  miniUnit: { fontSize: 10, color: '#888' },
  ...
  tabSwitcher: { flexDirection: 'row', backgroundColor: '#ffffff', borderRadius: 20, padding: 4, marginTop: 15 },
